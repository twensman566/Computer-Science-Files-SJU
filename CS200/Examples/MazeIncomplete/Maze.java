...
public class Maze implements GridColors {
  ...
  public boolean findMazePath(int col, int row) {
     ...
     */
    if(row <0 || col<0||maze.getNRows() >= row || maze.getNCols() >= col)
    {
      return false;
    }
    
    else if(maze.getColor(col,row) != (PATH))
    {
      return false;
    }
    else if(maze.getNRows()-1 == row || maze.getNCols()-1 == col)
    {
     maze.recolor(col,row,Color.GREEN);
     return true; 
    }
    else {
     maze.recolor(col,row,Color.GREEN);
    ...
    maze.recolor(col,row,Color.GRAY);
    return false;
  }
}