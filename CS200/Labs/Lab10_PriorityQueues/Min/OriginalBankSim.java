 ...
 */
public class OriginalBankSim {
  
  /** Queue of regular customers. */
  private CustomerQueue regularCustomerQueue = new CustomerQueue("Priority Customer");
  ...
  private ArrayList<Integer> customerServed = new ArrayList<Integer>();
  ...
  private ArrayList<Integer> numberServed = new ArrayList<Integer>();
  
  private ArrayList<Integer> idleTime = new ArrayList<Integer>();
  ...
  private void enterData() {
    ...
    System.out.println("Enter the highest priority possible for a customer (an integer): ");
    ...
    for (clock = 0; clock < totalTime; clock++) {
      ...
      for(int i = 0; i<timeDone.size(); i++)
      {
        if(timeDone.get(i) == clock){
          customerServed.set(busyTellerQueue.get(i),customerServed.get(i)+1);
      ...
      for(int i = 0; i<freeTellerQueue.size();i++)
      {
        if (!regularCustomerQueue.isEmpty()) { //means that teller is ready to serve (i.e., not busy)
          startServe();
        }
      }
      for(Integer i : freeTellerQueue){
        System.out.println("Time is " + clock + ": Teller number " + i + " is idle");
        this.idleTime.set(i,(this.idleTime.get(i)+1));
      }
    }
  }
  ...
  private void showStats() {
    ...
    for(int i =0; i<this.idleTime.size(); i++){
      System.out.println("Idle time for Teller " + i + " was: " + idleTime.get(i));
      System.out.println("Served " + customerServed.get(i) + " customers.");
      System.out.println();
      System.out.println();
    }
    System.out.println("Queue is now:");
    for(Customer cust : regularCustomerQueue.getElements())
    {  
      System.out.println("Customer number " + cust.getId() + " in queue with priority " + cust.getPriority() +
                         " and time stamp " + cust.getArrivalTime());
    }
  }
  ...
  public static void main(String args[] ){
    OriginalBankSim sim = new OriginalBankSim();
    sim.enterData();
    sim.runSimulation();
    sim.showStats();
  }
